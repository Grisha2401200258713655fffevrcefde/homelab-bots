...
- **перевод статей** — вытаскивает полный текст (не только RSS-сниппет) через
  экстрактор контента, переводит на русский через локальную Ollama, шлёт
  сначала перевод (🇷🇺), потом оригинал (🇬🇧). Русскоязычные источники (Habr)
  не переводятся — определяется автоматически по доле кириллицы в тексте
...
- **перевод статей** — работает "из коробки", если `OLLAMA_HOST` указывает на
  живую Ollama (по умолчанию hren-znaet:11434). Если Ollama недоступна —
  бот просто шлёт заголовок+ссылку как раньше, без перевода
