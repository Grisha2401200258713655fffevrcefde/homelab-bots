...
- **перевод статей** — вытаскивает полный текст (не только RSS-сниппет) через
  экстрактор контента, переводит на русский через **argos-translate (полностью
  ОФЛАЙН, модель скачана один раз при сборке образа, без ИИ/LLM и без сети)**,
  шлёт сначала перевод (🇷🇺), потом оригинал (🇬🇧). Русскоязычные источники
  (Habr) не переводятся — определяется автоматически по доле кириллицы в тексте.
  Медленнее облачных переводчиков на слабом CPU, зато никогда не виснет из-за сети
...
- `/bloat` — по SSH обходит все ноды, считает reclaimable место в Docker,
  в конце — краткое AI-резюме через Ollama: какие ноды реально стоит почистить
  и какую команду выполнить (если `OLLAMA_HOST` задан)
...
- **перевод статей** — работает "из коробки" без интернета вообще (модель
  скачивается один раз при сборке образа, дальше всё локально). Если перевод
  не нужен — `TRANSLATE_ENABLED=false`
